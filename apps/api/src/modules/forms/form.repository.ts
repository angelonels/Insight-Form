...
import type { EditableFormDraft, PublishedFormSchema, QuestionType } from "../../shared/types/form-schema.js";
import { createId, createPublicSlug } from "../../shared/utils/ids.js";
...
export class FormRepository {
  ...
  async listByOwner(ownerUserId: string) {
    const rows = await this.database.select().from(forms).where(eq(forms.ownerUserId, ownerUserId)).orderBy(desc(forms.updatedAt));
  ...
  async createBlank(ownerUserId: string, input: { title: string; description?: string | null }): Promise<FormDetail> {
  ...
  async updateMetadata(formId: string, ownerUserId: string, input: { title?: string; description?: string | null }) {
  ...
  async replaceDraft(formId: string, ownerUserId: string, draft: EditableFormDraft): Promise<FormDetail> {
  ...
  async addSection(formId: string, ownerUserId: string, input: { title?: string; description?: string | null; position?: number }) {
    await this.getOwnedForm(formId, ownerUserId);
    const [countRow] = await this.database.select({ count: count() }).from(formSections).where(eq(formSections.formId, formId));
    ...
    if (!section) {
      throw new ConflictError({ code: "SECTION_CREATE_FAILED", message: "Failed to create section." });
    }
  ...
  async updateSection(formId: string, ownerUserId: string, sectionId: string, input: { title?: string; description?: string | null; position?: number }) {
    await this.getOwnedForm(formId, ownerUserId);
    const [section] = await this.database
      .update(formSections)
  ...
  async deleteSection(formId: string, ownerUserId: string, sectionId: string) {
    await this.getOwnedForm(formId, ownerUserId);
    await this.database.delete(formSections).where(and(eq(formSections.id, sectionId), eq(formSections.formId, formId)));
  ...
  async addQuestion(
    ...
    const [countRow] = await this.database.select({ count: count() }).from(formQuestions).where(eq(formQuestions.sectionId, sectionId));
    ...
    if (!question) {
      throw new ConflictError({ code: "QUESTION_CREATE_FAILED", message: "Failed to create question." });
    }
  ...
  async deleteQuestion(formId: string, ownerUserId: string, questionId: string) {
    await this.getOwnedForm(formId, ownerUserId);
    const sectionIds = await this.getSectionIds(formId);
    await this.database.delete(formQuestions).where(and(eq(formQuestions.id, questionId), inArray(formQuestions.sectionId, sectionIds)));
  ...
  async duplicateQuestion(formId: string, ownerUserId: string, questionId: string) {
    ...
    if (!question) {
      throw new ConflictError({ code: "QUESTION_DUPLICATE_FAILED", message: "Failed to duplicate question." });
    }
  ...
  async reorderQuestions(formId: string, ownerUserId: string, sectionId: string, questionIds: string[]) {
    await this.getOwnedForm(formId, ownerUserId);
    const sectionIds = await this.getSectionIds(formId);
  ...
  async publish(formId: string, ownerUserId: string) {
    return this.database.transaction(async (tx) => {
      ...
      const detail = await this.getDetail(formId, ownerUserId);
      const questionCount = detail.sections.reduce((sum, section) => sum + section.questions.length, 0);
      if (!detail.sections.length || questionCount === 0) {
  ...
  async close(formId: string, ownerUserId: string) {
    await this.getOwnedForm(formId, ownerUserId);
    const [form] = await this.database
      .update(forms)
  ...
  private async getSectionIds(formId: string) {
    const rows = await this.database.select({ id: formSections.id }).from(formSections).where(eq(formSections.formId, formId));
  ...
  private async normalizeSectionPositions(formId: string) {
    ...
    for (const [index, section] of sections.entries()) {
      await this.database.update(formSections).set({ position: -(index + 1) }).where(eq(formSections.id, section.id));
    }
  ...
  private async normalizeQuestionPositions(sectionId: string) {
    ...
    for (const [index, question] of questions.entries()) {
      await this.database.update(formQuestions).set({ position: -(index + 1) }).where(eq(formQuestions.id, question.id));
    }
  ...
  async vectorSearch(formId: string, embedding: number[], limit = 12) {
